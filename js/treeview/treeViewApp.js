...
function openRecordEditorFor(personId) {
  if (window.opener && !window.opener.closed) {
    try {
      ...
      // window.opener.focus() лише "просить" браузер показати іншу вкладку —
      // на багатьох мобільних браузерах це навмисно ігнорується (безпека).
      // Натомість ЗАКРИТТЯ цієї вкладки (вона відкрита скриптом, тож може
      // закрити сама себе) практично завжди й надійно повертає фокус на
      // попередню — без прохань і без ризику, що користувач цього не помітить.
      showToast('✅ Відкриваю редактор у головній вкладці…');
      setTimeout(() => closeOrNavigateBack('index.html'), 700);
      return;
